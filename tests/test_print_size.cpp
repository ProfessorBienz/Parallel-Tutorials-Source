...
int main(int argc, char* argv[])
{
    ...
    char buffer[256];
    char np_str[16];  // buffer to hold the string version
    snprintf(np_str, sizeof(np_str), "%d", num_procs);
    int found = 0;

    while (fgets(buffer, sizeof(buffer), file)) {
        if (strstr(buffer, np_str) != NULL) {
            found = 1;
            break;
        }
    }
    ...
    if (!found)
    {
        fprintf(stderr, "Rank %d did not find process count.  Make sure you are printing Hello World, the rank, and the number of processes to pass Tutorial 01.\n", rank);
